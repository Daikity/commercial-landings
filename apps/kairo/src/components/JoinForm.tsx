...
import { joinSchema, type JoinValues } from '@/lib/join-schema';
...
export function JoinForm() {
  ...
  const {
    ...
    defaultValues: { name: '', email: '', practice: 'breath', note: '' },
  });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      <p className="rounded-[var(--radius-sm)] border border-[var(--color-line)] px-3 py-2 text-xs text-[var(--color-text-muted)]">
        Demo form — requests are mocked and not sent to a server.
      </p>
      ...
      <fieldset>
        <legend className="mb-2 text-sm font-medium">Practice</legend>
        <div className="grid gap-2 sm:grid-cols-2">
          {practices.map((option) => {
            const selected = practice === option.value;
            return (
              <label
                key={option.value}
        ...
        {errors.practice ? (
          <p className="mt-1 text-xs text-[var(--color-accent)]">{errors.practice.message}</p>
        ) : null}
      </fieldset>
      ...
      >
        {status === 'loading' ? 'Sending…' : 'Join a session'}
      </button>
