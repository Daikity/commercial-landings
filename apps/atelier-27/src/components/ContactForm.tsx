...
import { projectSchema, type ProjectValues } from '@/lib/project-schema';
...
export function ContactForm() {
  ...
  const {
    ...
    defaultValues: { name: '', email: '', type: 'residence', note: '' },
  });
    ...
    <form onSubmit={onSubmit} className="space-y-5" noValidate>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {projectTypes.map((opt) => {
            ...
            return (
              <label
                key={opt.value}
                className={`cursor-pointer border px-3 py-3 text-sm transition-colors focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-[var(--color-accent)] ${
                  selected
      ...
      <div>
        <label htmlFor="note" className="mb-1.5 block text-[10px] uppercase tracking-[0.2em]">
          Note <span className="normal-case tracking-normal text-[var(--color-text-muted)]">(optional)</span>
        </label>
      ...
      <button
        ...
        className="w-full border border-[var(--color-text)] bg-[var(--color-text)] px-5 py-3.5 text-xs uppercase tracking-[0.2em] text-[var(--color-bg)] hover:bg-transparent hover:text-[var(--color-text)] disabled:opacity-60"
      >
      ...
      {status === 'success' ? (
        <p className="text-sm text-[var(--color-success)]" role="status">
          Received (demo) — we will pretend to reply by email.
        </p>
      ) : null}
      {status === 'error' ? (
